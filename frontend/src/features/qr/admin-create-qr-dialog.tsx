...
export function AdminCreateQrDialog({ clientUuid }: { clientUuid: string }) {
  ...
  const createMutation = useMutation({
    mutationFn: (values: QrCreateFormValues) => adminQrCodesApi.create(clientUuid, values),
    onSuccess: (data) => {
      setResult(data);
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <Button onClick={() => setOpen(true)} disabled={!clientUuid}>
        ...
        {result ? (
          ...
          <form onSubmit={handleSubmit((values) => createMutation.mutate(values))} noValidate>
            <DialogHeader>
              ...
              <DialogDescription>Generates a scannable QR code linked to a destination for this client.</DialogDescription>
            </DialogHeader>
                ...
                <div className="space-y-2">
                  ...
                  {!reviewLocations?.length && (
                    <p className="text-xs text-muted-foreground">
                      This client has no Google Review locations yet - add one on their Google Reviews page first.
                    </p>
                  )}
                </div>
              ) : (

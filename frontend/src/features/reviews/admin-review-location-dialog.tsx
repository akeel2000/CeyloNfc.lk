...
export function AdminReviewLocationDialog({
  clientUuid,
  existing,
}: {
  clientUuid: string;
  existing?: GoogleReviewLocation;
}) {
  ...
  const mutation = useMutation({
    mutationFn: (values: ReviewLocationFormValues) =>
      isEdit
        ? adminReviewLocationsApi.update(existing!.uuid, values)
        : adminReviewLocationsApi.create(clientUuid, values),
    onSuccess: () => {
      toast.success(isEdit ? "Location updated" : "Location added");
      queryClient.invalidateQueries({ queryKey: ["admin", "google-reviews", clientUuid] });
      setOpen(false);
      reset();
    },
    onError: (error) => {

...
async function fetchProfile(slug: string): Promise<PublicProfile | null> {
  const response = await fetch(`${env.apiUrl}/public/profile/${slug}`, { cache: "no-store" });
...
export async function generateMetadata({
  ...
  return {
    title: profile.fullName ?? "Digital Profile",
    description: profile.bio ?? undefined,
  };
}

export default async function PublicIndividualProfilePage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  return (
    <>
      <ViewBeacon path={`/public/profile/${slug}/view`} />
      <PublicProfileCard profile={profile} vcardPath={`/public/profile/${slug}/vcard`} />
    </>
  );
}
